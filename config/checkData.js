...
function checkTeamNames(picks) {
    var pickArr = [];
    for (let i = 0; i < picks.west.length; i++){
        pickArr.push(picks.west[i])
    }
    pickArr.push(picks.champ)
    for (var i = 0; i < picks.east.length; i++){    // getting all picks into one array to simmplify next step
...
module.exports = {checkTeamNames, checkUsername}